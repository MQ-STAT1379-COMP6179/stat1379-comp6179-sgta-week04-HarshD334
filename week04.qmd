...
*Write your thoughts here:*
my_weather_data.csv file was created

Verification of the save, check data integrity, ensure reproducibility and early error detection
...
The first line of code they were using = instead of ==, hence got the error about named input.

The second line of code, here Sea is unquoted, so R looks for an object/variable named Sea and therefore the you get the object 'Sea' not found error
...
cholesterol_long <- cholesterol_data %>%
  pivot_longer(
    cols = everything(),       # take both oat_bran and oat_bran_free
    names_to = "diet",         # new column for group names
    values_to = "cholesterol"  # new column for values
  )
...
Oat bran group has a lower mean cholesterol (4.51) compared to the oat bran free group (4.79).

Oat bran free: sd = 0.592, values are a bit more consistent (less variability).

Oat bran: sd = 0.763, values are more spread out, meaning some individuals responded better (or worse) than others.
